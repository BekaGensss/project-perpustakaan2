...
<div class="container pt-4 pb-5">
    <div class="card shadow-sm border-0 mb-4" style="border-radius: 16px; overflow: hidden;">
        <div class="card-header bg-white border-0 pt-4 pb-3 px-4 d-flex justify-content-between align-items-center">
            <div>
                <h4 style="font-weight: 700; color: #0f172a; margin: 0;"><i class="fas fa-history text-primary mr-2"></i> Riwayat Peminjaman</h4>
                <p class="text-muted mt-1 mb-0" style="font-size: 14px;">Catatan historis semua buku yang pernah Anda pinjam sebelumnya.</p>
            </div>
        </div>
        
        <div class="card-body p-0">
            <div class="table-responsive">
                <table id="riwayat-pinjam" class="table table-modern mb-0" style="width:100%">
                    <thead>
                        <tr>
                            <th class="text-center" style="width: 50px;">#</th>
                            <th style="width: 90px;">Cover</th>
                            <th>Buku & Peminjaman</th>
                            <th>Jadwal Transaksi</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <img src="{{ asset('storage/' . $imagePath) }}" alt="Cover Buku" 
                                             style="width: 65px; height: 95px; object-fit: cover; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); filter: grayscale(20%);">
                                    </td>
                                    ...
                                    {{-- Jadwal Pinjam & Pengembalian --}}
                                    <td>
                                        <div class="info-sm">Lama Pinjam: <span class="val-sm">{{ $detail->lama_pinjam }} Hari</span></div>
                                        <div class="info-sm">Dikembalikan Tgl:</div>
                                        <div class="val-sm text-primary"><i class="fas fa-calendar-check"></i> {{ $detail->tgl_pengembalian ? date('d M Y', strtotime($detail->tgl_pengembalian)) : '-' }}</div>
                                    </td>
                                    ...
                                    <td>
                                        <div style="margin-bottom: 8px;">
                                            ...
                                            @else
                                                <span class="badge badge-success" style="padding: 6px 12px; font-weight: 600; border-radius: 6px; background-color: #10b981;"><i class="fas fa-check-circle mr-1"></i> Selesai</span>
                                            @endif
                                        </div>
                                        ...
                                        @php
                                            ...
                                            if (!is_null($detail->tgl_pengembalian)) {
                                                $tglPengembalian = \Carbon\Carbon::parse($detail->tgl_pengembalian)->startOfDay();
                                                if ($tglPengembalian->gt($tglKembali)) {
                                                    $terlambat = ceil($tglPengembalian->diffInDays($tglKembali, false));
                                        ...
                                        @if($terlambat > 0)
                                            ...
                                            <div class="info-sm">Total Denda: <span class="val-sm text-danger">Rp {{ number_format($totalDenda, 0, ',', '.') }}</span></div>
                                        @else
                                            <div class="info-sm text-success"><i class="fas fa-check"></i> Tepat waktu</div>
                                        @endif
                                    </td>

                                    {{-- Petugas --}}
                                    <td>
                                        <div class="info-sm" style="margin-bottom:6px;">
                                            <div class="text-xs text-muted mb-1">Diserahkan:</div>
                                            <div class="val-sm" style="font-size: 13px;">
                                                <img src="https://ui-avatars.com/api/?name={{ urlencode($pinjam->petugas_pinjam->nama) }}&background=6366f1&color=fff" style="width:18px;height:18px;border-radius:50%;margin-right:4px;">
                                                {{ $pinjam->petugas_pinjam->nama }}
                                            </div>
                                        </div>
                                        <div class="info-sm">
                                            <div class="text-xs text-muted mb-1">Diterima:</div>
                                            @if($detail->petugas_kembali)
                                            <div class="val-sm" style="font-size: 13px;">
                                                <img src="https://ui-avatars.com/api/?name={{ urlencode($detail->petugas_kembali->nama) }}&background=10b981&color=fff" style="width:18px;height:18px;border-radius:50%;margin-right:4px;">
                                                {{ $detail->petugas_kembali->nama }}
                                            </div>
                                            @else
...
<script>
    $(document).ready(function() {
        $("#riwayat-pinjam").DataTable({
            "responsive": true,
            "autoWidth": false,
            "ordering": false, /* Matikan order otomatis supaya yg terakhir tetap di atas jika querynya DESC */
            "language": {
